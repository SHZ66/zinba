...
foreach my $chrm (keys %{$filesOffsets}){
    ...
    &run_zinba($offsetFiles,$coordout,$winout,$formula,$threshold,$winsize,$method,$stdlog,$errlog,$printLog,$getRefinePeaks);
    $pm->finish;
}
...
sub run_zinba{
    my ($inputFile,$coordout,$winout,$formula,$threshold,$winSize,$method,$stdLog,$errLog,$printLog,$getRefinePeaks) = @_;
    my $off = $winSize/2;
    if ($printLog == 0){
